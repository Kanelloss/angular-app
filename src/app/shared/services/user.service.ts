import { inject, Injectable, signal, effect } from '@angular/core'; // signal and effect go together
...
export class UserService {
    ...
    user = signal<LoggedInUser | null>(null)  // (null) = initialize with value = null.

    // constructor runs once, when we call the component. ONLY if we have an 'effect' variable inside, it is "always" listening for any changes.
    constructor() {   
      const access_token = localStorage.getItem("access_token");  // gets value after refresh, so we don't lose the access token.
